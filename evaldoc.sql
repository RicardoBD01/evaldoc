...
COPY public.departamentos (id, nombre, creado_en) FROM stdin;
\.
...
COPY public.docentes (usuario_id) FROM stdin;
\.
...
COPY public.estudiantes (usuario_id, matricula) FROM stdin;
\.
...
COPY public.grupos_oferta (id, periodo_id, materia_id, grupo, docente_usuario_id, creado_en) FROM stdin;
\.
...
COPY public.import_lotes (id, periodo_id, archivo_nombre, archivo_hash, estado, errores_count, creado_en) FROM stdin;
\.
...
COPY public.import_registros (id, lote_id, row_num, departamento, materia_nombre, materia_clave, grupo, profesor_nombre, profesor_email, matricula, alumno_nombre, alumno_email, procesado, error, creado_en) FROM stdin;
\.
...
COPY public.inscripciones (id, grupo_oferta_id, estudiante_usuario_id, creado_en) FROM stdin;
\.
...
COPY public.jefaturas (id, usuario_id, departamento_id, periodo_id, activo, creado_en) FROM stdin;
\.
...
COPY public.materias (id, clave, nombre, departamento_id, creado_en) FROM stdin;
\.
...
COPY public.perfiles_usuario (usuario_id, nombre_completo, nombres, apellidos) FROM stdin;
\.
...
COPY public.periodos (id, codigo, fecha_inicio, fecha_fin, activo, creado_en) FROM stdin;
1	2025-1	\N	\N	t	2026-01-06 17:15:28.947788
\.
...
COPY public.usuarios (id, nombre, apaterno, amaterno, correo, pass, rol, activo) FROM stdin;
3	Lorelay	Martinez	Visairo	estudiante@cenidet.tecnm.mx	$2y$12$AQS6wTxU8KEBa0Tqncr6Nuej7Bfd1FuxQM6zm1qy7.9uLPgSsQzBK	3	t
1	admin	padmin	madmin	admin@admin.com	$2y$12$o.eZgmzB1.3EyBd9aqY9cOdUHOFQ6eE90oPoOSTx.2Vj4smKQzjsS	1	t
2	Ricardo	Barcenas	Dirzo	docentes@cenidet.tecnm.mx	$2y$12$C7eCnz7EzeNusbniC5JdF.LXD79HOnCORCShk8NleEIB9Hcamd8g2	2	t
4	prueba	dadd	daada	fs@fas.com	$2y$12$JWvGe7.OIqMo6QQ5M7SfCuyhU2mb5vQdKJ3.ncbcGkBqTIdkhTdei	3	f
\.
